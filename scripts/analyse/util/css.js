...
function css_extractor (folder_path, imports, getLocalModule) {
  const src = fs.readFileSync(path.join(folder_path, 'src/component.ts'), 'utf-8');
  ...
  for (let style of styles) 
  {
    ...
    for (let imp of imports[folder_path])
    {
      if (imp.name === name)
      {
        ...
        else 
        {
          ...
          try {
            const css = fs.readFileSync(path.resolve(folder_path, "dist/src", from), 'utf-8');
            variables = extract_cssvariables(css, folder_path);
          }
          catch (e)
          {
...
function extract_cssvariables(css, folder_path) {
