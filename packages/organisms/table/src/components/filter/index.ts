...
import { IColumn, FilterChangeEvent } from '../../types';
import { FilterIndividual } from "../filter-individual";

export class Filter extends Translator {
  static style = style;
  ...
  private handlesearch = (e: Event) => {
    if (e.target instanceof HTMLElement && 'value' in e.target) {
      ...
      this.filtersElement
        .querySelectorAll<FilterIndividual>('pap-table-filter-individual')
        .forEach(filter => {
  ...
  private handleApply = () => {
    if (!this.iscustomfilters) {
      ...
      this.filtersElement
        .querySelectorAll<FilterIndividual>('div.filter:not([hidden="true"]) > pap-table-filter-individual')
        .forEach(filter => {
    ...
    return html`
      <template>
        <div class="filter">
          ...
          <pap-table-filter-individual></pap-table-filter-individual>
        </div>
      </template>

      <main>
        <span class="dynamic">
          <pap-input @change="${this.handlesearch}" placeholder="${this.translateKey("Search...")}">
